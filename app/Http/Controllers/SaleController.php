...
class SaleController extends Controller
{
    ...
    {
        $query = Sale::whereNotNull('installer_id')
            ->where('installer_id', '!=', '');

        if ($date) {
            // Filter by specific date if provided
            $query->whereDate('installation_date', $date);
        } else {
            // Otherwise show all future installations
            $query->whereDate('installation_date', '>=', Carbon::now()->format('Y-m-d'));
        }

        $futureInstallations = $query
            ->orderBy('installation_date', 'asc')
            ->get();

        $selectedDate = $date; // can be null

        return view('pages.sale.calendar', compact('futureInstallations', 'selectedDate'));
    }
