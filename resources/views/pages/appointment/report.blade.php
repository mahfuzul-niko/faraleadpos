...
<div class="content">
    <div class="block block-rounded">
        ...
        <div class="row p-2">
            <div class="col-md-6"><h4 class="text-capitalize" id="transaction_type_title"> @if($date != 'all') {{date("d M Y", strtotime($date))}} @else All @endif {{$status}} @if($status != 'appointment') appointments @endif</h4></div>
        </div>
        
        <input type="hidden" name="" id="toggle_yes" value='1'>
        <input type="hidden" name="" id="date" value='{{$date}}'>
...
$(function () {
    var table = $('.data-table').DataTable({
        ...
        serverSide: true,
        ajax: {
            "url": "/admin/appointment-report-data/"+date+"/"+status,
        },
        columns: [
