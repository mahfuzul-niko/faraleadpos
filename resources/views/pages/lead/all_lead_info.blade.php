...
@section('body_content')
            ...
            <div class="block-content">
                    ...
                    <form method="get" action="{{ route('admin.all.lead.info') }}">
                        <div class="row">
                            <div class="form-group  @if (Auth::user()->type == 'admin') col-md-2 @else col-md-4 @endif">
                                ...
                                <input type="text" class="form-control rounded-pill" value="{{ request()->search }}"
                            ...
                            <div class="form-group col-md-2">
                                ...
                                <select class="form-control rounded-pill" name="status" id="lead_status">
                                    @foreach ($status as $key => $res)
                                        <option
                                            {{ (isset(request()->status) && request()->status == $res['name']) || (!isset(request()->status) && $key == 0) ? 'selected' : '' }}
                            ...
                            @if (Auth::user()->type == 'admin')
                                <div class="form-group col-md-2">
                                    ...
                                    <select class="form-control rounded-pill" name="assigned_to" id="lead_status">
                                        <option {{ !isset(request()->assigned_to) ? 'selected' : '' }} value="">
                                            Select</option>
                                        @foreach ($crm as $key => $res)
                                            <option
                                                {{ isset(request()->assigned_to) && request()->assigned_to == $res->id ? 'selected' : '' }}
                            ...
                            <div class="form-group col-md-2">
                                ...
                                <input type="date" class="form-control rounded-pill" value="{{ request()->start_date }}"
                                    name="start_date">
                            </div>
                            <div class="form-group col-md-2">
                                <label for="">End</label>
                                <input type="date" class="form-control rounded-pill" value="{{ request()->end_date }}"
                                    name="end_date">
                            </div>
                            <div class="form-group col-md-2">
                                <label for="">&nbsp;</label>
                    ...
                    <table class="table table-bordered data-table" id="lead_table">
                        <thead>
                            <tr>
                                <th>SN.</th>
                                <th width="10%">Date</th>
                                <th width="30%">Clients & Others Info</th>
                                <th>Assigned To</th>
                                ...
                                <tr class="@if (\Carbon\Carbon::parse(optional($lead)->created_at)->format('Y-m-d') == date('Y-m-d'))  @endif">
                                    ...
                                    <td class="LN{{ optional($lead)->id }}">
                                        <span class="lead-note"
                                            id="last_note_{{ optional($lead)->id }}">{{ optional($lead->last_note)->note }}</span><br>
                                    ...
                                    <td class="LS{{ optional($lead)->id }} text-center">
                                        @php
                                            $badgeColors = [
                                                'New' => 'bg-lightblue text-white',
                                                'Positive' => 'bg-success text-white', // green
                                                'Sale' => 'bg-primary text-white', // deep/navy blue
                                                'Cancel' => 'bg-danger text-white', // red
                                                'Flowup' => 'bg-lightyellow text-white',
                                                'Pending' => 'bg-warning text-black', // deep yellow
                                            ];
                                        @endphp

                                        <span class="lead-status" id="last_status_{{ optional($lead)->id }}">
                                            <span
                                                class="rounded-pill  px-3 py-1 font-bold {{ $badgeColors[optional($lead)->status] ?? 'bg-secondary text-white' }}">
                                    ...
                                    <td>
                                        <button data-toggle="modal"
                                            onclick="set_lead_note('{{ $lead->id }}', '{{ $lead->name }}', '{{ $lead->phone }}', '{{ $lead->address }}')"
                                        ...
                                        @if ($user->checkPermission('lead.update'))
                                            <a href="{{ route('admin.edit.lead', ['id' => $lead->id]) }}"
                                                class="btn btn-primary btn-sm btn-rounded">Edit</a>
                                        @endif
                                        @if ($user->checkPermission('lead.view'))
                                            <a title="View" target="_blank"
                                                href="{{ route('admin.view.lead', ['id' => $lead->id]) }}"
                                                class="btn btn-primary btn-sm"><i class="fas fa-eye"></i></a>
                                        @endif
                                        @if ($user->checkPermission('lead.status.update'))
                                            <button data-toggle="modal"
                                                onclick="set_lead_status('{{ $lead->id }}', '{{ $lead->name }}', '{{ $lead->phone }}', '{{ $lead->address }}', '{{ $lead->status }}')"
                                                data-target="#change_status_modal"
                                                class="btn btn-success btn-sm">Status</button>
                                        @endif
                                        {{-- <a target="_blank" type="button" href="{{route('admin.set.appiontment', ['id'=>$lead->id])}}" class="btn btn-dark btn-sm">App</a> --}}
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
                ...
                <div class="row">
                    ...
                    <div class="d-flex justify-content-center col-md-5">
                        <p class="text-bold">Showing {{ $leads->firstItem() }} to {{ $leads->lastItem() }} of
                            {{ $leads->total() }} results</p>
                    </div>
                </div>
    ...
    <!-- Lead Status Modal -->
    <div class="modal fade" id="change_status_modal" tabindex="-1" role="dialog" aria-labelledby="status_modalLabel"
        aria-hidden="true">
                ...
                <div class="modal-body">
                    <form action="javascript:void(0)" id="set_status_form" method="post">
                        ...
                        <div class="form-group">
                            ...
                            <h4><b class="text-success">Client Phone: </b> <span id="lead_status_client_phone"></span>
                            </h4>
                            <p><b class="text-success">Client Address: </b> <span id="lead_status_client_address"></span>
                            </p>
                        </div>
                        ...
                        <div class="block-content block-content-full text-right border-top">
                            ...
                            <input type="hidden" name="lead_id_for_lead_status" value=""
                                id="lead_id_for_lead_status">
    ...
    <!-- Lead Note Modal -->
    <div class="modal fade" id="lead_note_modal" tabindex="-1" role="dialog" aria-labelledby="lead_note_modalLabel"
        aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header bg-dark">
                ...
                <div class="modal-body">
                    <form action="javascript:void(0)" id="set_note_form" method="post">
                        ...
                        <div class="form-group">
                            ...
                            <p><b class="text-success">Client Address: </b> <span id="lead_note_client_address"></span>
                            </p>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="hidden" name="lead_id_for_lead_note" value=""
                                id="lead_id_for_lead_note">
                        </div>
                        <div class="block-content block-content-full text-right border-top">
                            <button type="button" class="btn btn-alt-primary mr-1" data-dismiss="modal">Close</button>
                            <button type="submit" class="btn btn-primary">Submit</button>
    ...
    <script type="text/javascript">
        ...
        function check_lead_note(note, className) {
            if ((note.indexOf('Number Off') !== -1) || (note.indexOf('Call Not Received') !== -1) || (note.indexOf(
                    'Call cut / busy') !== -1)) {
            ...
            } else if ((note.indexOf('Positive') !== -1) || (note.indexOf('Sale') !== -1) || (note.indexOf(
                    'Decision Pending - good') !== -1)) {
                //$('.'+className).addClass('bg-success text-light');
                $('.' + className).css({
                    'background-color': '#30C78D',
                    'color': '#ffffff'
                });
            } else if ((note.indexOf('Cancel') !== -1) || (note.indexOf('Not interested') !== -1) || (note.indexOf(
                    'Decision Pending - bad') !== -1) || (note.indexOf('Transfer To School') !== -1) || (note.indexOf(
                    'Transfer To Web') !== -1)) {
                    ...
                    $.ajax({
                        ...
                        beforeSend: function() {
                            $('#set_lead_note_loading').html(
                                '<div style="padding: 10px; text-align: center;">Submitting...<div>'
                            );
                        },
                        success: function(response) {
                            if (response['status'] == 'yes') {
                                $('#last_note_' + response['id']).html('<span>' + response[
                                    'note'] + '</span>');
                                $('#last_note_date_' + response['id']).html('<span>' +
                                    response['date'] + '</span>');
                    ...
                    $.ajax({
                        ...
                        beforeSend: function() {
                            $('#set_lead_status_loading').html(
                                '<div style="padding: 10px; text-align: center;">Updatting...<div>'
                            );
                        },
                        success: function(response) {
                            if (response['status'] == 'yes') {
                                location.reload();
                                $('#last_status_' + response['id']).html('<span>' +
                                    response['note'] + '</span>');
