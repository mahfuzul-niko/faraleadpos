...
<div class="content">
    ...
    <form action="javascript:void(0)" method="post" >
            @csrf
            <div class="row p-2">
                <div class="col-md-6"><h4 id="transaction_type_title">All Leads</h4></div>
            ...
            <table class="table table-bordered data-table">
                <thead>
                    <tr>
                        <th>Name</th>
                        <th>Phone</th>
                        <th width="20%">Query & Others Info</th>
                        <th>Last Note</th>
                        <th>Status</th>
...
<div class="modal fade" id="lead_note_modal" tabindex="-1" role="dialog" aria-labelledby="lead_note_modalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form action="{{route('set.lead.note')}}" method="post">
        @csrf
        <div class="form-group">
            <h4><b class="text-success">Client Name: </b> <span id="lead_note_client_name"></span></h4>
        </div>
        <div class="form-group">
            <label for="example-text-input"><span class="text-danger">*</span>Note</label>
            <textarea class="form-control" name="note" id="" cols="30" rows="4" required></textarea>
...
$(function () {
    var table = $('.data-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('admin.all.leads.data') }}",
        columns: [
            {data: 'name', name: 'name'},
            {data: 'phone', name: 'phone'},
            {data: 'query_and_others', name: 'query_and_others'},
            {data: 'note', name: 'note'},
...
function set_lead_note(id, name) {
